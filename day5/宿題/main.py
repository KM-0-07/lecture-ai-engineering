...
class ModelTester:
    ...
    @staticmethod
    def compare_with_prev_model(current_metrics, prev_metrics, tolerance=0.02):
        """過去バージョンのモデルと比較する"""
        return current_metrics["accuracy"] >= prev_metrics["accuracy"] - tolerance
...
def test_model_performance():
    ...
    try:
        # 過去バージョンのモデルの読み込み
        prev_model = ModelTester.load_model()
        prev_metrics = ModelTester.evaluate_model(prev_model, X_test, y_test)
        # 過去バージョンのモデルと比較
        assert ModelTester.compare_with_prev_model(
            metrics, prev_metrics
        ), f"モデル性能が過去バージョンのモデルのスコアを下回っています: {metrics['accuracy']}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 過去バージョンのモデルとの比較
        prev_comparison_ok = ModelTester.compare_with_prev_model(metrics, prev_metrics)
        print(
            f"過去バージョンとのモデル比較: {'合格' if prev_comparison_ok else '不合格'}"
        )
    except FileNotFoundError:
        pass
